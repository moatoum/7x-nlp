...
export function generateSeedData(count: number = 32): Submission[] {
  const now = Date.now();
  const submissions: Submission[] = [];

  for (let i = 0; i < count; i++) {
    // Distribute dates: more recent submissions are more likely
    ...
    submissions.push({
      id: randomId(),
      referenceNumber: randomRef(date),
      status: pick(STATUSES),
      createdAt,
      entityType: pick(['A business', 'A governmental entity']),
      ...
      currentCourier: Math.random() > 0.3 ? pick(['EMX', 'Aramex', 'DHL', 'FedEx', 'Zajel']) : null,
      supplierStatus: null,
      ...
      contactPhone: null,
      companyName: company,
      tag: Math.random() > 0.6 ? pick(['NXN', 'EMX']) : null,
      recommendedServices: [],
