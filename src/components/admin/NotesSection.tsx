...
export function NotesSection({ submissionId, notes }: NotesSectionProps) {
  ...
  const canAddNotes = useAdminUiStore((s) => s.canAddNotes);
  const username = useAdminUiStore((s) => s.username);
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
      ...
      <div className="p-5 space-y-4">
        {/* Input form — only for admin role */}
        {canAddNotes() ? (
          ...
          <p className="text-[11px] text-gray-300 text-center py-1">View-only access</p>
        )}
