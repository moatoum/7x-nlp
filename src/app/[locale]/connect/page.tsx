...
export default function ConnectPage() {
  ...
  function validate(): boolean {
    ...
    if (!form.contactName.trim()) errs.contactName = t('connect.errNameRequired');
    if (!form.businessEmail.trim()) errs.businessEmail = t('connect.errEmailRequired');
    else if (!EMAIL_REGEX.test(form.businessEmail)) errs.businessEmail = t('connect.errEmailInvalid');
    if (!form.phone.trim()) errs.phone = t('connect.errPhoneRequired');
    else if (!isValidPhone(form.phone)) errs.phone = t('connect.errPhoneInvalid');
    if (!form.entityName.trim()) errs.entityName = t('connect.errEntityRequired');
    setErrors(errs);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setErrors({ form: data.error || t('connect.errGeneric') });
        return;
      }
      setSubmitted(true);
    } catch {
      setErrors({ form: t('connect.errSubmitFailed') });
    } finally {
      setSubmitting(false);
    }
  }
    ...
    return (
      <div className="min-h-screen bg-[#fafafa] flex items-center justify-center px-4">
        ...
        >
          ...
          <h2 className="text-xl font-semibold text-gray-900 mb-2">{t('connect.submitted')}</h2>
          <p className="text-sm text-gray-400 mb-6">{t('connect.expertReachOut')}</p>
          <LocaleLink
            href="/"
    ...
    <div className="min-h-screen bg-[#fafafa]">
      <header className="sticky top-0 z-10 bg-white/80 backdrop-blur-md border-b border-gray-100">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-center relative">
          <LocaleLink href="/" className="absolute start-4 flex items-center gap-2 text-sm text-gray-500 hover:text-gray-900 transition-colors">
            <ArrowLeft className="w-4 h-4 rtl:rotate-180" />
            {t('connect.back')}
          </LocaleLink>
      ...
      <main className="max-w-lg mx-auto px-4 py-12">
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="text-xl font-semibold text-gray-900">{t('connect.title')}</h1>
              <p className="text-[13px] text-gray-400">{t('connect.subtitle')}</p>
            </div>
          </div>

          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('connect.fullName')}</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('connect.businessEmail')}</label>
              <input
                type="email"
                value={form.businessEmail}
                onChange={(e) => setForm({ ...form, businessEmail: e.target.value })}
                placeholder={t('connect.placeholderEmail')}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('connect.phoneNumber')}</label>
              <input
                type="tel"
                value={form.phone}
                onChange={(e) => setForm({ ...form, phone: e.target.value })}
                placeholder={t('connect.placeholderPhone')}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('connect.entityName')}</label>
              <input
                type="text"
                value={form.entityName}
                onChange={(e) => setForm({ ...form, entityName: e.target.value })}
                placeholder={t('connect.placeholderEntity')}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">{t('connect.uaeRegistered')}</label>
